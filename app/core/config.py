...
def merge_configs(default: dict, user: dict) -> dict:
    """Merge configuración del usuario con la por defecto (recursivo)."""
    result = default.copy()
...
class Settings:
    ...
    def __init__(self) -> None:
        ...
        self.MAX_UPLOAD_SIZE = 5 * 1024 * 1024 * 1024  # 5 GB
        ...
        # -------------------------------
        #
        # Se leen desde variables de entorno, por ejemplo:
        #   SMTP_SERVER=smtp.gmail.com
        #   SMTP_PORT=587
        #   SMTP_USERNAME=tu_usuario@gmail.com
        #   SMTP_PASSWORD=tu_password_o_app_password
        #   SMTP_FROM_EMAIL=tu_usuario@gmail.com
        #
        # Además se mantienen alias smtp_user / email_from para no romper
        # código existente que los esté usando.

        self.smtp_server: str | None = os.environ.get("SMTP_SERVER", "smtp.example.com")
        self.smtp_port: int = int(os.environ.get("SMTP_PORT", "587"))

        # Acepta tanto SMTP_USERNAME como SMTP_USER (compatibilidad)
        self.smtp_username: str | None = (
            os.environ.get("SMTP_USERNAME") or os.environ.get("SMTP_USER") or "render_user"
        )
        self.smtp_password: str | None = os.environ.get("SMTP_PASSWORD", "secure_password")

        # Acepta tanto SMTP_FROM_EMAIL como EMAIL_FROM (compatibilidad)
        self.smtp_from_email: str | None = (
            os.environ.get("SMTP_FROM_EMAIL")
            or os.environ.get("EMAIL_FROM")
            or "render-manager@local.com"
        )

        # Alias internos por si en alguna parte del código existente
        # se usan estos nombres antiguos:
        self.smtp_user = self.smtp_username
        self.email_from = self.smtp_from_email

    # ----- Métodos de acceso / actualización de config -----
    ...
    def save_config(self) -> bool:
        """Guardar configuración actual en disco."""
    ...
    def validate_config(self) -> Dict[str, Any]:
        """Validar configuración actual y devolver issues/warnings."""
        issues = []
        warnings = []
        ...
        if not blender_config["path"] and not blender_config["auto_detect"]:
            issues.append("Blender no está configurado (ni path ni auto_detect).")
        ...
        if render_config["max_concurrent_jobs"] > 10:
            warnings.append(
                "max_concurrent_jobs muy alto, puede afectar el rendimiento."
            )
