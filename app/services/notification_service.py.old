...
# Importa la configuración global (clase Settings de app/core/config.py)
...
class NotificationService:
    def __init__(self) -> None:
        ...
        # -----------------------------
        # Usamos getattr para evitar AttributeError si el atributo no existe
        self.smtp_server = getattr(settings, "smtp_server", None)
        self.smtp_port = getattr(settings, "smtp_port", 587)

        # Compatibilidad: smtp_user / smtp_username
        self.email_user = (
        ...
        # Compatibilidad: smtp_from_email / email_from
        self.email_from = (
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, auth=auth, data=data) as response:
                    if response.status in (200, 201):
                        ...
                        print(
                            f"❌ Error enviando WhatsApp: status={response.status} body={text}"
                        )
                        return False
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=data) as response:
                    if response.status == 200:
                        ...
                        text = await response.text()
                        print(
                            f"❌ Error enviando Telegram: status={response.status} body={text}"
                        )
                        return False
    ...
    # ------------------------------------------------------------------
    # NOTIFICACIONES DE ALTO NIVEL (jobs / nodos)
    # ------------------------------------------------------------------
